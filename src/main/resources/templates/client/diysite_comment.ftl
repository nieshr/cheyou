<div id="comment-list"> 
    <menu class="pro_eval w100">
        ...
        <h3 class="tit">
            <input type="radio" <#if !stars?? || stars?? && stars==0>checked="checked"</#if> onclick="javascript:getCommentByStars(${diysiteId?c}, 0, 0);"/><span>全部评价（${comment_count!'0'}）</span>
            <input type="radio" <#if stars?? && stars==3>checked="checked"</#if> onclick="javascript:getCommentByStars(${diysiteId?c}, 3, 0);"/><span>好评（${three_star_comment_count!'0'}）</span>
            <input type="radio" <#if stars?? && stars==2>checked="checked"</#if> onclick="javascript:getCommentByStars(${diysiteId?c}, 2, 0);"/><span>中评（${two_star_comment_count!'0'}）</span>
            <input type="radio" <#if stars?? && stars==1>checked="checked"</#if> onclick="javascript:getCommentByStars(${diysiteId?c}, 1, 0);"/><span>差评（${one_star_comment_count!'0'}）</span>
        </h3>
            ...
            <li>
                ...
                <div>
                    ...
                    <p>
                     ...
                     <p class="clear h10"></p>
                    <p>
                        <span class="fl ml20">同盟店综合印象：</span>
                        <#if item.compositeStar?? && item.compositeStar gt 0>
                            <#list 1..item.compositeStar as starCount>
                                <a class="fl" href="javascript:;"><img src="/client/images/content/start01.png" /></a>
                            </#list>
                            <#if 5-item.compositeStar gt 0>
                                <#list 1..(5-item.compositeStar) as whiteCount>
                                    <a class="fl" href="javascript:;"><img src="/client/images/content/start03.png" /></a>
                                </#list>
                            </#if>
                        <#else>
    ...
    <div class="darkpage">
        ...
        <#else>
            <a href="javascript:getCommentByStars(${diysiteId?c}, ${stars!'0'}, ${comment_page.number-1});">上一页</a>
        </#if>
        ...
        <#if comment_page.totalPages gt 0>
            <#list 1..comment_page.totalPages as page>
                <#if page <= 3 || (comment_page.totalPages-page) < 3 || (comment_page.number+1-page)?abs<3 >
                    ...
                    <#else>
                        <a href="javascript:getCommentByStars(${diysiteId?c}, ${stars!'0'}, ${page-1});">${page}</a>
                    </#if>
                    <#assign continueEnter=false>
                <#else>
                    <#if !continueEnter>
        ...
        <#else>
            <a class="pn-next" href="javascript:getCommentByStars(${diysiteId?c}, ${stars!'0'}, ${comment_page.number+1});">下一页</a>
        </#if>
        ...
        <div>
            ...
            <input type="submit" class="sub" onclick="javascript:getCommentByStars(${diysiteId?c!''}, ${stars!'0'}, parseInt($('#pageIndex').val())-1);" value="确定" />
            <p class="clear"></p>
        </div>
    </div>
    <div class="clear h20"></div>
